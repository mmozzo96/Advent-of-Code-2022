...
const startingMonkeys: monkey[] = Data.map((description) =>
  readStartingMonkey(description)
);
...
const tests: number = new Array(Data.length).fill(1)
  .map((one: number, idx: number) => {
    return +Data[idx][2].slice('  Test: divisible by '.length)
  }).reduce((sF: number, n: number) => sF*n)

function solution2(startingMonkeys: monkey[], testMultiplied: number) {
  let numberOfInspections: number[] = new Array(startingMonkeys.length).fill(0);

  function newOperation(operation: operation, item: number): number {
    if(item>testMultiplied) item = item%testMultiplied
  ...
  async function monkeyTurn(monkeys: monkey[], idx: number) {
    ...
    for (let i = 0; i < itemsLength; i++) {
      const item: number = <number>items.shift();
      const itemAfterInspection: number = newOperation(operation, item);
      const testResult: boolean = test(itemAfterInspection);
      const itemIsMovedTo: number = toMonkey(testResult);
  ...
  async function logSolution() {
    await doNRound(10000, startingMonkeys);
    const max1Idx = findIdxMax(numberOfInspections);
    const max1 = numberOfInspections[max1Idx];
    numberOfInspections.splice(max1Idx, 1);
    const max2 = Math.max(...numberOfInspections);
...
solution2(startingMonkeys, tests);
